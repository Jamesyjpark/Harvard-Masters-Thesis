...
for(kk in 1:length(betaVI)){
  ...
  for(ss in 1:nSim){
    ...
    for(dd in 1:nDay){
      stepD[,dd]<-rowSums(steps[,((1:nObs)+nObs*(dd-1))]) 
    }
    ...
    #steps per subject
    matLMM[,"Steps"]<-as.vector(t(steps))
...
################################################################################ 
pdf(file=paste("PLOTnObs",nObs,"baseStep",floor(baseStep),"stepCut",stepCut,"VarNoise",0,"MeanNoise",0,".pdf",sep=""))
